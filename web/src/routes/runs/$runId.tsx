...
import { getLocalRun } from "@/api/projects";
...
function LocalRunDetailPage() {
  const runId =
    window.location.pathname.split("/runs/")[1]?.split("/")[0] || "";

  const { data: run, isLoading: runLoading } = useQuery({
    queryKey: ["local-run", runId],
    queryFn: () => getLocalRun(runId),
    refetchInterval: (query) => {
      // Poll every 2s while running to catch initial data
